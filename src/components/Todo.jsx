...
function Todo({todo}){
    ...
    return(
        <div className={`${todo.completed ? 'bg-green-300' : ''} bg-gray-400 w-[100%] md:w-[80%]  mx-auto py-2 px-1 md:px-2 text-xl text-black relative my-2`}>
            ...
            <input type="text" className={` bg-transparent w-[70%] md:w-[80%] ${todo.completed ? 'line-through' : ''}`} defaultValue={todo.todo_text} onChange={(e) => setUpdatedTodo({...Todo,todo_text : e.target.value})} />
